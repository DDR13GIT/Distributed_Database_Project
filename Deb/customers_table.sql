CREATE OR REPLACE PROCEDURE insert_customer(
    ...
    p_name IN Customers_Deb.customer_name%TYPE,
    p_email IN Customers_Deb.email%TYPE,
    p_phone IN Customers_Deb.phone%TYPE
  ) IS
  BEGIN
    IF p_customer_id BETWEEN 1 AND 1000 THEN
      INSERT INTO Customers_Noumi@site_link (customer_id, customer_name, email, phone)
      VALUES (p_customer_id, p_name, p_email, p_phone);
    ELSE
      INSERT INTO Customers_Deb (customer_id, customer_name, email, phone)
      VALUES (p_customer_id, p_name, p_email, p_phone);
    END IF;
  END insert_customer;
/
...
CREATE OR REPLACE PROCEDURE update_customer(
  ...
  BEGIN
    IF p_customer_id BETWEEN 1 AND 1000 THEN
      UPDATE Customers_Noumi@site_link SET phone = p_phone where customer_id = p_customer_id;
      
    ELSE
      UPDATE Customers_Deb SET phone = p_phone where customer_id = p_customer_id;
    END IF;
  END update_customer;
/
