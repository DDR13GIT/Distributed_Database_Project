...
BEGIN
    ...
    IF v_seat_availability = 'AVAILABLE' THEN
        ...
        INSERT INTO Tickets_Noumi@site_link (ticket_id, seat_id, customer_id, event_id, purchase_date, price)
...
-- exec Insert_Ticket(1,1,1,1,'20-Feb-2023', 50);

-- CREATE OR REPLACE PROCEDURE delete_ticket(
--     p_ticket_id IN NUMBER
-- )
-- IS
--     v_seat Seats_Deb_1.seat_id%TYPE;
-- BEGIN

--     -- Get the seat ID associated with the ticket
--     SELECT seat_id INTO v_seat FROM Tickets_Deb WHERE ticket_id = p_ticket_id;

--     -- Delete the ticket
--     DELETE FROM Tickets_Deb WHERE ticket_id = p_ticket_id;
