...
BEGIN
    ...
    FROM (
        (SELECT price FROM Tickets_Noumi WHERE event_id = p_event_id) UNION ALL (SELECT price FROM Tickets_Deb@site_link WHERE event_id = p_event_id)
    );
...
DECLARE
    v_event_id Events_Noumi.event_id%TYPE;
    v_event_name Events_Noumi.event_name%TYPE;
    v_num_tickets NUMBER(10);
    v_total_revenue NUMBER(10, 2);
    CURSOR c_events IS SELECT event_id, event_name FROM Events_Noumi;
BEGIN
    FOR c IN c_events LOOP
        v_event_id := c.event_id;
        v_event_name := c.event_name;
        SELECT COUNT(*) INTO v_num_tickets
        FROM (SELECT * FROM tickets_noumi UNION SELECT * FROM tickets_deb@site_link)
